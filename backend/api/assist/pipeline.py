...
def submit_answer_task(task: TaskPayload) -> bool:
    ...
    _submit_cfg = get_config()
    if _is_asr_task(task):
        question = str(task[0] or "")
        # Step 1: fast deterministic cleanup (zero latency, no LLM call).
        if bool(getattr(_submit_cfg, "assist_question_cleanup_enabled", True)):
            try:
                from services.stt.text_utils import cleanup_asr_question_text
                cleaned = cleanup_asr_question_text(question)
                if cleaned and cleaned != question:
                    _ilog.info(
                        "ANSWER_QUESTION_CLEANED before=%r after=%r",
                        question[:120], cleaned[:120],
                    )
                    question = cleaned
            except Exception as e:
                _elog.debug("question cleanup skipped: %s", e)
        # Step 2: optional LLM rewrite/correction (deep cleanup, one LLM call).
        if _should_run_asr_llm_correction(_submit_cfg, _task_meta(task)):
            try:
                from services.review_analysis import correct_question_text
                corrected = correct_question_text(question)
                if corrected and corrected != question:
                    _ilog.info(
                        "ANSWER_QUESTION_CORRECTED before=%r after=%r",
                        question[:120], corrected[:120],
                    )
                    question = corrected
            except Exception as e:
                _elog.debug("question correction skipped: %s", e)
        if question != str(task[0] or ""):
            task = (question, task[1], task[2], task[3], task[4])
...
def _should_run_asr_llm_correction(cfg, meta: dict[str, Any]) -> bool:
    """Reserve blocking LLM correction for uncertain, unstructured ASR text.

    Semantic parsing already gives high-confidence clusters deterministic
    cleanup.  Correcting each such cluster with another model call adds cost
    and serial latency before the answer can even enter the scheduler.
    """
    if not bool(getattr(cfg, "assist_answer_correction_enabled", False)):
        return False
    cluster = meta.get("question_cluster")
...
def flush_pending_question_group() -> bool:
    """Commit the currently assembled interviewer question immediately."""
    session = get_session()
    cfg = get_config()
    with _asr_state_lock:
        had_pending = _asr_state.pending_group is not None
        _asr_state.try_flush_question_group(cfg, session, time.monotonic(), True)
    return had_pending


def discard_pending_question_group() -> bool:
    """Discard the current provisional question without touching transcript history."""
    with _asr_state_lock:
        had_pending = _asr_state.pending_group is not None
        _asr_state.reset_pending_group()
    if had_pending:
...
def pending_question_group_snapshot() -> dict[str, Any]:
    with _asr_state_lock:
        group = _asr_state.pending_group
        if group is None:
            return {"pending": False, "utterances": [], "raw_text": ""}
        return {
            "pending": True,
            "utterances": list(group.utterances),
            "raw_text": " ".join(group.utterances),
            "source": group.source,
            "has_promote": group.has_promote,
        }
...
def _note_interviewer_speech_activity(now_mono: float) -> None:
    with _asr_state_lock:
        _asr_state.note_speech_activity(now_mono)
...
def _judge_asr_question(text: str) -> bool:
    """用 LLM 快速判断一句不确定的转写是否真是提问；失败时退回 True（不拦截）。"""
    try:
        ...
        prompt = (
            "下面是一句面试场景中的语音转写文本。判断这句话是否在向候选人提问"
            "（包括请求介绍/解释/设计/分析/给方案等指令式提问）。寒暄、独白、"
            "单纯陈述（如“我们开始吧”“这道题不难”）不算提问。只输出 是 或 否。\n\n"
            f"文本：{(text or '')[:300]}"
        )
        kwargs = _completion_token_kwargs(model_cfg, 8)
        think = _build_think_params(model_cfg, SimpleNamespace(think_mode=False, think_effort="off", max_tokens=8))
        if think:
        ...
        return ("是" in content) or content.lower().startswith("yes") or content == "1"
    except Exception:
        return True


def _maybe_llm_judge_pending_asr(cfg) -> None:
    """对队列里第一个「不确定」的 ASR 自动题做 LLM 判题：非问题直接剔除，避免误答。"""
    if not bool(getattr(cfg, "assist_llm_question_detect_enabled", True)):
        return
    with _dispatch_lock:
        found_idx = None
        found_item = None
        for i, item in enumerate(_pending):
            meta = _task_meta(item[0])
            if meta.get("origin") == "asr" and meta.get("asr_kind") == "candidate":
                found_idx = i
                found_item = item
                break
        if found_idx is None:
            return
        task, seq, tv = found_item
    text = (task[0] or "").strip() if isinstance(task, tuple) else ""
    if not text:
        return
    is_question = _judge_asr_question(text)
    with _dispatch_lock:
        try:
            cur = _pending[found_idx]
        except IndexError:
            return
        if cur != found_item:
            return
        if is_question:
            meta = dict(_task_meta(task))
            meta["asr_kind"] = "promote"
            _pending[found_idx] = ((task[0], task[1], task[2], task[3], meta), seq, tv)
        else:
            _pending.pop(found_idx)
            _mark_seq_skipped(seq)
            _ilog.info("ASR_LLM_JUDGE reject=%r", text[:80])
...
def _try_dispatch():
    while True:
        ...
        cfg = get_config()
        _maybe_llm_judge_pending_asr(cfg)
...
def _dedup_final_transcription(
    text: str,
    last_text: str,
    last_at: float,
    now_mono: float,
    ...
    """最终转录去重：窗口内重复（含前缀/后缀重叠）直接丢弃，防止同句二次触发自动回答。

    参考 TechSpar / VoiceWings 的 1.2s 窗口策略。
    """
    if not text:
        return text, False
    if last_text and now_mono - last_at <= window_sec:
        if text == last_text or last_text.endswith(text) or text.endswith(last_text):
            return "", True
    return text, False


def _maybe_emit_copilot_hint(text: str) -> None:
    """面试官问题定稿后，若已激活策略树则做轻量匹配并推送追问/高危提示。"""
    try:
        from services.copilot_strategy import get_active_tree, match_question
        current_session = get_session()
        tree = get_active_tree(current_session.session_id)
        if not tree or not (text or "").strip():
            return
        hint = match_question(tree, text, current_session.session_id)
        if hint and hint.get("node_id"):
            broadcast({"type": "copilot_hint", **hint})
...
def _interviewer_asr_worker(runtime: _InterviewerRuntime, session, cfg) -> None:
    ...
    while True:
        ...
        try:
            ...
            if pub:
                now_mono = time.monotonic()
                pub, is_dup = _dedup_final_transcription(
                    pub, last_final_text, last_final_at, now_mono
                )
                if is_dup:
                    _ilog.debug("INTERVIEWER_ASR_DEDUP text=%r", pub[:120])
                elif pub:
                    last_final_text = pub
                    last_final_at = now_mono
                    _ilog.info("ASR publish=%r", pub[:120])
                    _append_transcription_fragment(
                        current_cfg, session, pub, now_mono, False
                    )
                    _maybe_emit_copilot_hint(pub)
        except Exception as e:
...
def _interview_worker():
    ...
    # Doubao 流式采用「开流一次、连续喂」：段切换只 reset 展示，不重连 WS，
    # 让每个新段第一个字立刻出 partial（避免每次握手 ~100-300ms 的延迟）。
    dstream = None
    dstream_is_doubao = False
    last_fed_len = 0
    live_chunks: list[np.ndarray] = []
    live_fed = 0
    live_feed_gate = max(0.001, float(getattr(cfg, "silence_threshold", 0.01) or 0.01) * 0.3)
    ...
    def _on_interviewer_stream_error(message: str) -> None:
        try:
            broadcast({
                "type": "stt_stream_error",
                "provider": "doubao",
                "message": f"豆包实时转录连接中断，将在分句后自动重试：{str(message)[:100]}",
            })
        except Exception:
            pass

    def _ensure_interviewer_stream() -> None:
        nonlocal dstream, dstream_is_doubao
        if dstream is not None:
            return
        dstream_is_doubao = False
        cfg = get_config()
        provider = (getattr(cfg, "stt_provider", "whisper") or "whisper").strip()
        # 豆包凭据可用时优先走云端流式（实时逐字），失败再回退本地 whisper 流式。
        has_doubao_cred = bool(
            getattr(cfg, "doubao_stt_api_key", "")
            or getattr(cfg, "doubao_stt_access_token", "")
        )
        if provider == "doubao" or (provider == "whisper" and has_doubao_cred):
            from services.stt.doubao_stream import DoubaoStreamSession
            try:
                eng = get_stt_engine(provider="doubao")
                ...
                dstream.start()
                dstream_is_doubao = True
                return
            except Exception as e:
                _elog.debug("interviewer doubao stream open failed: %s", e)
                _on_interviewer_stream_error(str(e))
                dstream = None
        if not bool(getattr(cfg, "whisper_stream_enabled", True)):
            return
        from services.stt.whisper_stream import WhisperStreamSession
        try:
            ...
            dstream = WhisperStreamSession(
                ...
                min_window_sec=max(0.2, float(getattr(cfg, "whisper_stream_min_sec", 0.6) or 0.6)),
                max_window_sec=max(2.0, float(getattr(cfg, "whisper_stream_window_sec", 8.0) or 8.0)),
            )
            dstream.start()
        except Exception as e:
            _elog.debug("interviewer whisper stream open failed: %s", e)
            dstream = None

    def _feed_live_doubao(vad_chunk: np.ndarray) -> None:
        """只要有可听信号就喂给豆包流（不等 VAD 确认），边说边出字。"""
        nonlocal dstream, dstream_is_doubao, live_fed
        if _pause_event.is_set():
            return
        if dstream is None:
            _ensure_interviewer_stream()
            if not dstream_is_doubao:
                return
        live_chunks.append(vad_chunk)
        total = sum(len(c) for c in live_chunks)
        if total <= live_fed:
            return
        audio = np.concatenate(live_chunks) if len(live_chunks) > 1 else live_chunks[0]
        try:
            dstream.feed(audio[live_fed:])
            live_fed = len(audio)
        except Exception as e:
            _elog.debug("interviewer doubao live feed failed: %s", e)
        # 丢弃已消费的前缀，控制内存
        while live_chunks and len(live_chunks[0]) <= live_fed:
            live_fed -= len(live_chunks[0])
            live_chunks.pop(0)
        max_keep = int(AudioCapture.SAMPLE_RATE * 30)
        total = sum(len(c) for c in live_chunks)
        if total > max_keep:
            drop = total - max_keep
            while live_chunks and drop > 0:
                c0 = live_chunks[0]
                if len(c0) <= drop:
                    drop -= len(c0)
                    live_fed = max(0, live_fed - len(c0))
                    live_chunks.pop(0)
                else:
                    live_chunks[0] = c0[drop:]
                    live_fed = max(0, live_fed - drop)
                    drop = 0
                    ...
                    if speech_audio is None:
                        ...
                        if (
                            ...
                            try:
                                ...
                                if pending is not None and len(pending) > last_fed_len:
                                    if dstream is None:
                                        _ensure_interviewer_stream()
                                    if dstream is not None:
                                        dstream.feed(pending[last_fed_len:])
                                        last_fed_len = len(pending)
                            except Exception as e:
...
def _candidate_streaming_config(cfg, provider: str) -> tuple[bool, int]:
    # Whisper 走间隔批量 partial；Doubao 走 DoubaoStreamSession 实时增量 feed，
    # 不在此重复批量转写（避免双份云成本）。
    enabled = bool(getattr(cfg, "candidate_streaming_asr_enabled", True)) and provider == "whisper"
    interval_ms = max(300, min(5000, int(getattr(cfg, "candidate_streaming_asr_interval_ms", 400) or 400)))
...
def _candidate_worker():
    ...
    live_feed_gate = max(
        0.001,
        float(getattr(get_config(), "silence_threshold", 0.01) or 0.01) * 0.3,
    )

    def _on_candidate_partial(text: str) -> None:
        try:
            broadcast({"type": "candidate_transcription_partial", "text": text})
        except Exception:
            pass

    def _feed_candidate_live_doubao(vad_chunk: np.ndarray) -> None:
        """候选人侧豆包连续流：只要有可听信号就喂，不等 VAD 确认。"""
        nonlocal dstream, dstream_is_doubao, live_fed
        if _pause_event.is_set():
            return
        if dstream is None:
            try:
                from services.stt.doubao_stream import DoubaoStreamSession
                dstream = DoubaoStreamSession(
                    get_stt_engine(provider="doubao"),
                    on_partial=_on_candidate_partial,
                )
                dstream.start()
                dstream_is_doubao = True
            except Exception as exc:
                _elog.debug("Candidate doubao live stream open failed: %s", exc)
                return
        live_chunks.append(vad_chunk)
        total = sum(len(c) for c in live_chunks)
        if total <= live_fed:
        ...
        try:
            ...
            _elog.debug("Candidate doubao live feed failed: %s", exc)
        while live_chunks and len(live_chunks[0]) <= live_fed:
            live_fed -= len(live_chunks[0])
            live_chunks.pop(0)
        max_keep = int(AudioCapture.SAMPLE_RATE * 30)
    ...
    def _finalize_candidate_audio(final_audio, log_kind: str) -> None:
        nonlocal current_segment_id, last_partial_text, last_partial_at, dstream, last_fed_len, wstream, last_wfed_len, last_final_text, last_final_at, dstream_is_doubao, live_fed
        ...
        try:
            ...
            if pub:
                ...
                if is_dup:
                    _ilog.debug("CANDIDATE_ASR_DEDUP %s text=%r", log_kind, pub[:120])
                elif pub:
                    last_final_text = pub
                    last_final_at = now_mono
    ...
    try:
        while not _stop_event.is_set():
            ...
            if not chunks:
                ...
                continue
            try:
                cand_energy = AudioCapture.compute_energy(np.concatenate(chunks))
                broadcast({"type": "candidate_audio_level", "value": round(cand_energy, 4)})
            except Exception:
                pass
            ...
            pending_audio = None
            speech_audio = None
            cand_provider, cand_model, cand_language, cand_allow_remote = _candidate_provider_config(cfg)
            for chunk in chunks:
                for vad_chunk in _iter_vad_feed_chunks(chunk):
                    if (
                        cand_provider == "doubao"
                        and AudioCapture.compute_energy(vad_chunk) > live_feed_gate
                    ):
                        _feed_candidate_live_doubao(vad_chunk)
            ...
            if getattr(vad, "has_pending_audio", False):
                ...
                use_whisper_stream = (
                    provider == "whisper"
                    and bool(getattr(cfg, "whisper_stream_enabled", True))
                )
                now_mono = time.monotonic()
                pending_audio = vad.pending_audio() if hasattr(vad, "pending_audio") else None
                if (
                    not use_whisper_stream
                ...
                # 豆包实时预览已由上方 _feed_candidate_live_doubao 连续流处理
                # Local streaming Whisper preview: background
                # sliding-window partials; never blocks the capture loop.
                if use_whisper_stream and pending_audio is not None and current_segment_id:
                    try:
                        if wstream is None:
                            from services.stt.whisper_stream import WhisperStreamSession
                            lang = (language or "").strip()
                            stream_lang = lang if lang and lang != "auto" else "zh"
                            wstream_engine = get_stt_engine(
                                provider="whisper",
                                model_size=(model or "base").strip(),
                                language=stream_lang,
                            )
                            wstream = WhisperStreamSession(
                                model_size=(model or "base").strip(),
                                language=stream_lang,
                                position=getattr(cfg, "position", "后端开发"),
                                engine=wstream_engine,
                                on_partial=_on_candidate_partial,
                                interval_sec=max(
                                    0.12,
                                    float(getattr(cfg, "whisper_stream_interval_ms", 280) or 280) / 1000.0,
                            ...
                            wstream.start()
                        if len(pending_audio) > last_wfed_len:
                            wstream.feed(pending_audio[last_wfed_len:])
                            last_wfed_len = len(pending_audio)
                    except Exception as exc:
                        _elog.debug("Candidate whisper stream feed failed: %s", exc)
            if speech_audio is None:
                continue

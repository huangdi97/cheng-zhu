...
from .pipeline import (
    ...
    stop_interview_loop,
    submit_answer_task,
    flush_pending_question_group,
    discard_pending_question_group,
    pending_question_group_snapshot,
    unpause_interview,
    _bump_generation,
)
...
@router.get("/question-boundary")
async def api_question_boundary_status():
    return pending_question_group_snapshot()


@router.post("/question-boundary/flush")
async def api_question_boundary_flush():
    return {"ok": True, "flushed": flush_pending_question_group()}


@router.post("/question-boundary/discard")
async def api_question_boundary_discard():
    return {"ok": True, "discarded": discard_pending_question_group()}
...
class ScreenRegionRequest(BaseModel):
    ...
    width: int = Field(default=100, ge=10, le=8000)
    height: int = Field(default=100, ge=10, le=8000)


@router.post("/ask-from-server-screen-region")
async def api_ask_from_server_screen_region(body: ScreenRegionRequest):
    ...
    try:
        data_url = await run_in_threadpool(
            ...
            getattr(cfg, "screen_capture_max_long_edge", 1600),
        )
    except Exception as e:
        _rlog.warning("capture region failed: %s", e, exc_info=True)
        raise HTTPException(503, f"截取自定义区域失败: {e}")
    text = prompt_server_screen_code(cfg.language, "custom")
    if pick_model_index((text, data_url, True, "server_screen_region", {"origin": "server_screen"}), set()) is None:
        raise HTTPException(503, "没有可用的识图模型，请先在设置中配置并启用支持视觉的模型")
    queued = submit_answer_task((text, data_url, True, "server_screen_region", {"origin": "server_screen"}))
    if not queued:
        raise HTTPException(503, "答题队列已满，请稍后再试")
    return {"ok": True}
...
@router.post("/assist/memo")
async def api_update_memo(body: MemoUpdateRequest):
    """更新手动面试备忘（与自动滚动摘要一起展示并注入回答上下文）。"""
    from api.realtime.ws import broadcast

    session = get_session()
    session.memo_manual = (body.text or "").strip()
    broadcast({
        "type": "memo_update",
        "system_summary": session.system_summary or "",
        "memo_manual": session.memo_manual or "",
    })
    return {"ok": True}

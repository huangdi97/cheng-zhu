...
def _mask_stt_key(value: str) -> str:
    return STT_KEY_MASK if (value or "").strip() else ""
...
def build_config_payload(cfg) -> dict:
    ...
    return {
        ...
        "doubao_stt_access_token": _mask_stt_key(cfg.doubao_stt_access_token or ""),
        "doubao_stt_api_key": _mask_stt_key(getattr(cfg, "doubao_stt_api_key", "") or ""),
        ...
        "generic_stt_api_key": _mask_stt_key(getattr(cfg, "generic_stt_api_key", "") or ""),
        ...
        "candidate_streaming_asr_interval_ms": max(300, min(5000, int(getattr(cfg, "candidate_streaming_asr_interval_ms", 400) or 400))),
        ...
        "assist_auto_answer_mode": str(getattr(cfg, "assist_auto_answer_mode", "smart") or "smart"),
        ...
        "assist_answer_correction_enabled": bool(getattr(cfg, "assist_answer_correction_enabled", False)),
        "assist_question_cleanup_enabled": bool(getattr(cfg, "assist_question_cleanup_enabled", True)),
        "assist_realtime_think_enabled": bool(getattr(cfg, "assist_realtime_think_enabled", False)),
        "assist_realtime_concise_answer": bool(getattr(cfg, "assist_realtime_concise_answer", True)),
        "assist_llm_question_detect_enabled": bool(getattr(cfg, "assist_llm_question_detect_enabled", True)),
        "assist_realtime_concise_max_tokens": max(160, min(2048, int(getattr(cfg, "assist_realtime_concise_max_tokens", 480) or 480))),
        ...
        "jd_text": getattr(cfg, "jd_text", None) or "",
        "interview_notes": getattr(cfg, "interview_notes", None) or "",
        "answer_language": (getattr(cfg, "answer_language", "中文") or "中文"),
        "assist_answer_align_jd_enabled": bool(getattr(cfg, "assist_answer_align_jd_enabled", False)),
        "assist_inline_translation_enabled": bool(getattr(cfg, "assist_inline_translation_enabled", False)),

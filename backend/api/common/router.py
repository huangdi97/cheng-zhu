...
class ConfigUpdate(BaseModel):
    ...
    assist_auto_answer_mode: Optional[str] = None
    ...
    assist_answer_correction_enabled: Optional[bool] = None
    assist_question_cleanup_enabled: Optional[bool] = None
    assist_realtime_think_enabled: Optional[bool] = None
    assist_realtime_concise_answer: Optional[bool] = None
    jd_text: Optional[str] = None
    interview_notes: Optional[str] = None
    answer_language: Optional[str] = None
    assist_answer_align_jd_enabled: Optional[bool] = None
    assist_inline_translation_enabled: Optional[bool] = None
    assist_suggestion_enabled: Optional[bool] = None
    assist_prefix_cache_warmup_enabled: Optional[bool] = None
    rolling_memo_enabled: Optional[bool] = None
    ...
    try:
        if "assist_auto_answer_mode" in d:
            ...
            if mode not in {"off", "smart", "always"}:
                raise HTTPException(422, "自动回答模式必须是 off、smart 或 always")
            d["assist_auto_answer_mode"] = mode
            d["auto_detect"] = mode != "off"
        elif "auto_detect" in d:
            d["assist_auto_answer_mode"] = "smart" if bool(d["auto_detect"]) else "off"
        if "models" in d:
            raw_models = []
        ...
        if "candidate_streaming_asr_interval_ms" in d:
            d["candidate_streaming_asr_interval_ms"] = max(300, min(5000, int(d["candidate_streaming_asr_interval_ms"])))
        ...
        # STT 密钥：前端不回显明文，掩码/空值表示「保持不变」
        from api.common.config_payload import STT_KEY_MASK as _stt_mask
        for _key in ("doubao_stt_access_token", "doubao_stt_api_key", "generic_stt_api_key"):
            if _key in d and d[_key] in ("", _stt_mask):
                d.pop(_key, None)
...
async def api_devices():
    try:
        ...
        platform_info = AudioCapture.get_platform_info(devices=devices)

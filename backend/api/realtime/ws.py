...
def _sessions_payload():
    try:
        from core.session import list_sessions, session_id
...
async def websocket_endpoint(ws: WebSocket):
    ...
    try:
        ...
        await ws.send_json({
            ...
            "system_summary": snapshot.get("system_summary", "") or "",
            "memo_manual": snapshot.get("memo_manual", "") or "",
            "session_id": snapshot.get("session_id", "") or "",
            "label": snapshot.get("label", "") or "",
            "sessions": _sessions_payload()["items"],
            "active_id": _sessions_payload()["active_id"],
        })
        while True:

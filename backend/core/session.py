...
class Session:
    ...
    system_summary: str = ""
    # 手动面试备忘（用户可编辑；与自动摘要一起注入 <memo_context> 并展示）
    memo_manual: str = ""
    ...
    def get_conversation_messages_for_llm(
        ...
        *,
        ...
        if include_summary and (self.system_summary or self.memo_manual):
            memo_parts = []
            if self.memo_manual:
                memo_parts.append("你手动记录的面试备忘(必须遵守,优先于自动摘要):\n" + self.memo_manual)
            if self.system_summary:
                memo_parts.append("以下是之前面试中已经发生的问答的滚动摘要(由系统压缩,仅供你了解上下文,不要复读它):\n" + self.system_summary)
            out.append({
                "role": "system",
                "content": "\n\n".join(memo_parts),
            })
...
# 多场面试会话隔离：每个 session 独立转录/问答/备忘；get_session() 始终返回当前激活场次。
...
def get_session() -> Session:
    """返回当前激活的会话；首次调用时创建默认会话。"""
    global _session, _sessions, _active_session_id
    with _lock:
        if _session is None:
            _active_session_id = _default_session_id()
            _session = Session(session_id=_active_session_id, label="默认会话")
            _sessions[_active_session_id] = _session
        return _session


def session_id() -> str:
    return get_session().session_id or _default_session_id()
...
def list_sessions() -> list[dict]:
    ...
    for s in sessions:
        out.append({
            "id": s.session_id or _default_session_id(),
            "label": s.label or "未命名会话",
            "created_at": s.created_at,
            "qa_count": len(s.qa_pairs),
            "transcription_count": len(s.transcription_history),
            "is_active": (s.session_id or _default_session_id()) == _active_session_id,
            "is_recording": s.is_recording,
            "is_paused": s.is_paused,
        })
    out.sort(key=lambda x: (not x["is_active"], x["created_at"]))
    return out


def create_session(label: str = "") -> Session:
    """新建一场独立会话并切换为激活状态。"""
    global _session, _sessions, _active_session_id
    import uuid
    with _lock:
        sid = uuid.uuid4().hex[:12]
        sess = Session(session_id=sid, label=(label or "").strip() or f"面试 {len(_sessions) + 1}")
        _sessions[sid] = sess
...
def activate_session(session_id_to_activate: str) -> Optional[Session]:
    global _session, _active_session_id
    with _lock:
        target = _sessions.get(session_id_to_activate)
        if target is None:
            return None
        _active_session_id = session_id_to_activate
        _session = target
        return target


def delete_session(session_id_to_delete: str) -> bool:
    global _sessions
    with _lock:
        if session_id_to_delete == _active_session_id:
            return False
        if session_id_to_delete not in _sessions:
            return False
        del _sessions[session_id_to_delete]
        return True


def rename_session(session_id_to_rename: str, label: str) -> bool:
    with _lock:
        target = _sessions.get(session_id_to_rename)
        if target is None:
            return False
...
def snapshot_session() -> dict:
    with conversation_lock:
        ...
        snap["session_id"] = session.session_id or _default_session_id()
        snap["label"] = session.label or ""
        return snap

...
def main() -> None:
    try:
        ...
        sys.exit(2)
    region_arg = (sys.argv[1] or "left_half").strip() if len(sys.argv) > 1 else "left_half"
    try:
    ...
    try:
        with mss.mss() as sct:
            if len(sct.monitors) < 2:
                ...
                sys.exit(3)
            mon = sct.monitors[1]
            if region_arg.startswith("{"):
                try:
                    rect = json.loads(region_arg)
                    left = max(mon["left"], int(rect.get("left", mon["left"])))
                    top = max(mon["top"], int(rect.get("top", mon["top"])))
                    right = min(mon["left"] + mon["width"], left + max(1, int(rect.get("width", 0))))
                    bottom = min(mon["top"] + mon["height"], top + max(1, int(rect.get("height", 0))))
                    region = {
                        "left": left,
                        "top": top,
                    ...
                    region = _region_for_monitor(mon, "left_half")
            else:
                region_kind = region_arg

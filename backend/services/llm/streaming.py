...
class LLMInsufficientBalance(LLMError):
    user_msg = "大模型账户余额不足，请充值或切换可用模型。"
...
def _classify_exception(e: BaseException) -> LLMError:
    ...
    message = str(e).lower()
    if status == 402 or "insufficient balance" in message or "余额不足" in message:
        return LLMInsufficientBalance(_compact_error_detail(e), cause=e)
    ...
    if isinstance(e, openai.BadRequestError):
        if "context" in message or "maximum context" in message or "token" in message:
...
def warmup_prefix_cache(messages, model_cfg=None, timeout_s=8.0):
    """Send a 1-token request to warm the provider's prefix KV cache.

    The message prefix (system prompt + stable history) is identical to the
    following real request, so DeepSeek-style prefix caching makes the real
    first token arrive faster.  Never raises: answer path must not be blocked.
    """
    try:
        cfg = get_config()
        if not bool(getattr(cfg, "assist_prefix_cache_warmup_enabled", True)):
            return {"ok": False, "skipped": "disabled"}
        if model_cfg is None:
            model_cfg = cfg.get_active_model()
        if not (getattr(model_cfg, "api_key", "") and getattr(model_cfg, "api_base_url", "") and getattr(model_cfg, "model", "")):
            return {"ok": False, "skipped": "no_model"}
        prefix = [m for m in (messages or []) if m and m.get("content")]
        if not prefix:
        ...
        hit = int(getattr(usage, "prompt_cache_hit_tokens", 0) or 0) if usage else 0
        _log.info(
            "PREFIX_CACHE_WARMUP model=%s messages=%d hit_tokens=%d",
            model_cfg.name, len(prefix), hit,
        )
        return {"ok": True, "prompt_cache_hit_tokens": hit}
    except Exception as e:  # noqa: BLE001
        _log.debug(
            "PREFIX_CACHE_WARMUP skip model=%s err=%s",
            getattr(model_cfg, "name", "?"),
            _compact_error_detail(e),
        )
        return {"ok": False, "skipped": str(e)[:160]}


def schedule_prefix_cache_warmup(messages, model_cfg=None):
    """Fire-and-forget warmup in a daemon thread; never blocks the answer path."""
    try:
        if not messages:
            return
        t = threading.Thread(
            target=warmup_prefix_cache,
            args=(messages, model_cfg),

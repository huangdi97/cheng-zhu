...
_SUMMARY_PROMPT = """你是面试会话的记忆压缩助手。请把下面这段更早的「问答历史」提炼成结构化滚动备忘，用于后续回答保持一致、不前后矛盾。
严格按三个小节输出（每个小节用 Markdown 二级标题）：
## 已问过的问题
- 列出面试官已经问过的问题主题（一句话一个，不要重复细节）
## 我已声称的事实
- 候选人/你提到过的关键技能栈、项目、量化数字、结论（防止后续自相矛盾）
## 面试官关注点
- 面试官追问的方向、反复强调的重点
...
输出要求：
- 中文, Markdown 列表。
- 每个小节不超过 6 条 bullet；每条不超过 60 字，不复述原文。
    ...
    def _worker() -> None:
        ...
        with conversation_lock:
            try:
                ...
                try:
                    from api.realtime.ws import broadcast as _ws_broadcast
                    _ws_broadcast({
                        "type": "memo_update",
                        "system_summary": new_summary,
                        "memo_manual": getattr(target_session, "memo_manual", "") or "",
                    })

...
def correct_question_text(question: str) -> str:
    """Use the active LLM to rewrite an ASR question into a clean, complete question."""
    ...
    try:
        ...
        return text
    system = (
        "你是一位严谨的语音识别（ASR）纠错助手。"
        "输入是语音转文字得到的面试问题，"
        "可能有错字、漏字、同音字错误、口语填充词和缺失的标点。"
        "请只输出修正后的、规范通顺的面试问题本身；"
        "不要解释过程，不要回答问题。"
    )
    prompt = (
        "下面是语音识别（ASR）得到的面试问题，请修正：\n"
        "1) 纠正错字/漏字/同音词（如“减力”→“简历”、“自动价值”→“自动驾驶”、“circle”→“SQL”）；\n"
        "2) 删除口语填充词（嗯、呃、那个、就是说、然后呢等）；\n"
        "3) 补全/修正技术术语（Redis、Kafka、MySQL、Docker、Kubernetes 等）；\n"
        "4) 补齐标点：如果是提问，句末用“？”；\n"
        "5) 只输出修正后的问题本身，不要回答，不要加解释。\n\n"
        "待修正的问题：\n" + text
    )
    try:
        response = client.chat.completions.create(
            ...
            temperature=0.1,
            **_review_chat_kwargs(600),
        )
        content = response.choices[0].message.content
        corrected = (content or "").strip()
        if not corrected or len(corrected) < len(text) * 0.7:
            return text
        if SequenceMatcher(None, text, corrected).ratio() < 0.4:
            return text
        return corrected
    except Exception as e:
        logger.warning("correct_question_text failed: %s", e)
        return text
...
# ---------------------------------------------------------------------------
# 录音复盘：把一段连续转写文本整理成 面试官/候选人 多轮问答
# ---------------------------------------------------------------------------
...
def structure_transcript_turns(raw_text: str, max_turns: int = 30) -> list[dict[str, str]]:
    """用 LLM 把一段面试录音转写文本整理成多轮问答。

    返回 [{question, answer}, ...]；失败返回空列表。
    """
    ...
    try:
        ...
        return []
    prompt = (
        "你是一个面试录音转写整理助手。下面是一段面试录音的语音识别文本"
        "（可能没有标点、有口语词、没有角色标注）。\n"
        "请把它整理成多轮问答：\n"
        "- 按内容判断说话人：提问方 = 面试官，回答方 = 候选人；寒暄/独白可跳过；\n"
        "- 修正明显错别字与专业术语，去掉“嗯、那个、就是”等语气词，补上标点；\n"
        "- 一句里既有问题又有回答时，按最近一次提问归并成一轮；\n"
        f"- 最多 {max_turns} 轮，只输出 JSON。\n\n"
        "输出严格 JSON 格式：\n"
        '{"turns": [{"question": "面试官的问题", "answer": "候选人的回答"}]}\n\n'
        "转写文本：\n"
        f"{raw[:6000]}"
    )
    try:
        response = client.chat.completions.create(
            model=model_name,
            messages=[
                {"role": "system", "content": "你是一位严谨的面试复盘整理助手，只输出 JSON。"},
                {"role": "user", "content": prompt},
            ],
            temperature=0.2,
            **_review_chat_kwargs(2000),
        )
        ...
        logger.warning("structure_transcript_turns failed: %s", e)
        return []

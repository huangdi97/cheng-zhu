...
def update_turn_text(
    ...
    """编辑一轮 turn 的问题/回答文本，并把该轮分析重置为 pending。

    保留 original_candidate_answer_text 以便追溯；清空旧分析字段。
    """
    now = time.time()
    with _db_lock:
        conn = _conn()
        cur = conn.execute(
            ...
            SET question_text = ?,
                ...
                scorecard_json = '{}',
                updated_at = ?
            WHERE session_id = ? AND qa_id = ?
            """,
            (question_text or "", candidate_answer_text or "", now, session_id, qa_id or ""),
        )
...
def delete_turn(session_id: int, qa_id: str) -> bool:
    """删除一轮 turn，并刷新 session 的 turn_count。"""
    now = time.time()
    with _db_lock:
        conn = _conn()
        cur = conn.execute(
            "DELETE FROM review_turns WHERE session_id = ? AND qa_id = ?",
            (session_id, qa_id or ""),
        )
        changed = cur.rowcount > 0
        if changed:
...
def recent_profile(limit: int = 10) -> dict:
    ...
    供练习闭环使用：下一场练习按这些弱项优先出题。
    """
    limit = _bounded_int(limit, fallback=10, minimum=1, maximum=100)
    with _db_lock:
        conn = _conn()
        rows = conn.execute(
            """
            SELECT strong_points_json, weak_points_json, behavior_traits_json, domain_summary_json
            FROM review_sessions
            WHERE status IN ('completed', 'analyzing')
            ORDER BY started_at DESC
            LIMIT ?
            """,
            (limit,),
    ...
    for row in rows:
        for s in _load_list(row["strong_points_json"]):
            if isinstance(s, str) and s.strip() and s not in strengths:
                strengths.append(s.strip())
        for w in _load_list(row["weak_points_json"]):
            if isinstance(w, str) and w.strip() and w not in weaknesses:
                weaknesses.append(w.strip())
        for b in _load_list(row["behavior_traits_json"]):
            if isinstance(b, str) and b.strip() and b not in behavior_traits:
                behavior_traits.append(b.strip())
        # 领域掌握度：取最近几场的域级信息求平均
        for domain, score in _load_dict(row["domain_summary_json"]).items():
            try:
                domain_mastery[str(domain)] = float(score)
            except (TypeError, ValueError):
                pass
    ...
    return {
        ...
        "behavior_traits": behavior_traits[:12],

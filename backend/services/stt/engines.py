...
def _build_ws_frame_full_request(
    ...
    *,
    ...
    end_window_size: int = 400,
    force_to_speech_time: int = 1000,
    enable_accelerate_text: bool = True,
    accelerate_score: int = 5,
    enable_ddc: bool = True,
    enable_punc: bool = True,
    enable_itn: bool = True,
    show_utterances: bool = True,
) -> bytes:
    """Build the full client request frame for Doubao streaming ASR.

    - bigmodel_async (双向流式优化版) 推荐参数: enable_nonstream 二遍识别,
      result_type=single 增量返回, end_window_size 提前判停 (min 200ms),
      enable_accelerate_text 首字加速, enable_ddc 语义顺滑。
    - 注意: audio.language 仅 bigmodel_nostream 支持, 双向流式(含 async)不传
      language 时默认支持中英文+方言自动识别, 传了反而可能报错。
    """
    audio: dict[str, Any] = {
        "format": "pcm",
        "rate": 16000,
    ...
    request: dict[str, Any] = {
        ...
        "enable_nonstream": bool(enable_nonstream),
        "result_type": result_type if result_type in ("full", "single") else "single",
        "show_utterances": bool(show_utterances),
    }
    if enable_nonstream:
        request["end_window_size"] = max(200, int(end_window_size or 400))
        if force_to_speech_time and int(force_to_speech_time) > 0:
            request["force_to_speech_time"] = max(1, int(force_to_speech_time))
    if enable_accelerate_text and int(accelerate_score or 0) > 0:
        request["enable_accelerate_text"] = True
        request["accelerate_score"] = max(0, min(20, int(accelerate_score or 0)))
    req: dict[str, Any] = {
        "user": {"uid": app_key},
        "audio": audio,
        "request": request,
...
def _parse_doubao_result_payload(payload: dict) -> dict:
    """Normalize a full-server response payload into streaming-friendly fields.

    Returns dict with keys:
      - text:     result.text (whole / incremental recognized text)
      - definite: list of definite (sentence-final, nostream 二遍) utterance texts
      - partial:  latest non-definite utterance text (live partial)
    """
    res = payload.get("result") or {}
    if not isinstance(res, dict):
        return {"text": "", "definite": [], "partial": ""}
    ...
    if isinstance(utterances, list):
        for u in utterances:
            ...
            if u.get("definite"):
                ...
                partial = ut
    return {"text": text, "definite": definite, "partial": partial}
...
class DoubaoSTT:
    ...
    def transcribe(self, audio: np.ndarray, sample_rate: int = 16000,
        ...
        if websocket is None:
            raise RuntimeError("豆包流式识别依赖 websocket-client，当前环境未安装")
            ...
            try:
                ...
                while True:
                    ...
                    if msg_type == MSG_FULL_SERVER_RESPONSE and payload:
                        ...
                        if info["definite"]:
                            definite_parts = info["definite"]
                if definite_parts:
                    final_text = "".join(definite_parts) or final_text
...
class STTEngine:
    ...
    def load_model(self):
        with self._lock:
            ...
            try:
                ...
                self._device = device
                try:
                    # Warm up CUDA/cuDNN kernels so the first real decode is fast
                    warm = np.zeros(1600, dtype=np.float32)
                    warm_segments, _ = self._model.transcribe(
                        warm, language="zh", beam_size=1, vad_filter=False
                    )
                    list(warm_segments)
                except Exception as exc:
                    if device == "cuda" and self._is_cuda_runtime_error(exc):
                        _log.warning("Whisper CUDA warmup failed; falling back to CPU: %s", exc)
                        self._model = WhisperModel(
                            self.model_size,
                            device="cpu",
                            compute_type="int8",
    ...
    def _is_cuda_runtime_error(exc: BaseException) -> bool:
        ...
        return any(token in message for token in ("cuda", "cublas", "cudnn"))

    def _fallback_to_cpu(self) -> None:
        """Replace a CUDA model that failed at first real inference."""
        with self._lock:
    ...
    def _transcribe_with_runtime_fallback(self, audio_f, hotwords_str: str, kwargs: dict):
        def _call():
            try:
                segments, info = self._model.transcribe(audio_f, hotwords=hotwords_str, **kwargs)
            except TypeError:
                segments, info = self._model.transcribe(audio_f, **kwargs)
            # faster-whisper defers the actual encoder call until this
            # generator is consumed.  Materialize it inside the fallback
            # boundary so missing CUDA DLLs are caught here, not by the caller.
        ...
        try:
            ...
            if self._device != "cuda" or not self._is_cuda_runtime_error(exc):
                raise
            _log.warning("Whisper CUDA inference failed; retrying on CPU: %s", exc)
    ...
    def transcribe_fast(
        ...
        """Fast partial decode for streaming previews: beam_size=1, no VAD filter.

        Uses the same hotword/initial-prompt machinery as :meth:`transcribe`
        but trades a little accuracy for much lower latency, which is what
        live partial output needs. The authoritative batch path still calls
        :meth:`transcribe` (beam_size=3).
        """
        if self._model is None:
            self.load_model()

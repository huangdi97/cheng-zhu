...
_ASR_FILLER_LEAD = re.compile(
    r"^(?:(?:嗯|呃|啊|哎|额|嗯嗯|呃呃|那个|这个|就是说呢|就是说|就是|其实|那么|好的|好)[，,。\s]*)+"
)
_ASR_FILLER_TRAIL = re.compile(
    r"(?:[，,。\s]*(?:那个|这个|就是说呢|就是说|然后呢|然后|就是|其实|那么|好的|嗯|呃|啊|哎|额))+\s*$"
)
_ASR_DOUBLED_INTERJ = re.compile(
    r"(?<![A-Za-z0-9])(?:嗯嗯|呃呃|啊啊|哎哎|额额)(?![A-Za-z0-9])"
)
_ASR_REPEAT_PUNCT = re.compile(r"([，。？！?、])\1+")
_ASR_WS = re.compile(r"\s+")


def cleanup_asr_question_text(text: str) -> str:
    """Fast deterministic cleanup of an ASR question before it goes to the LLM.

    Strips spoken fillers (leading/trailing), doubled interjections and repeated
    punctuation. No LLM, no latency, safe to run on every question.
    """
    t = (text or "").strip()
    if not t:
        return t
    ...
    while changed and t:
        changed = False
        m = _ASR_FILLER_LEAD.match(t)
        if m:
            t = t[m.end():].lstrip()
            changed = True
    t = _ASR_FILLER_TRAIL.sub("", t).strip()
    t = _ASR_DOUBLED_INTERJ.sub("", t)
    t = _ASR_REPEAT_PUNCT.sub(r"\1", t)
    t = _ASR_WS.sub(" ", t)
    return t.strip()

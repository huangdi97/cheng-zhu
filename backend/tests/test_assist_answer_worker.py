...
def _cfg():
    return SimpleNamespace(
        ...
        assist_realtime_concise_answer=False,
    )
...
def test_personal_experience_question_buffers_and_replaces_ungrounded_model_claim(
    ...
    def fake_stream(_model, messages, **_kwargs):
        ...
        model_called = True
        yield ("text", "我确实做过 Kafka 项目，负责过高吞吐消息链路。")
    ...
    monkeypatch.setattr(answer_worker, "chat_stream_single_model", fake_stream)
    monkeypatch.setattr(answer_worker, "schedule_prefix_cache_warmup", fake_warmup)

    answer_worker.process_question_parallel(
        ("你做过 Redis 吗？", None, False, "asr", {"origin": "asr", "asr_turn_id": 1}),
        seq=0,
        model_idx=0,
        sess_v=0,
    ...
    assert start["fact_grounding"]["status"] == "no_profile"
    assert start["model_name"] == "模型一"
    assert model_called is True
    assert warmup_called is True
    assert len(chunks) == 1
    assert "Redis" in chunks[0]
    assert "Kafka" not in chunks[0]
    assert "没有直接做过 Redis" in done["answer"]
    assert "通用方法" in done["answer"]


def test_personal_experience_with_evidence_uses_fast_natural_fact_voice(
    monkeypatch: pytest.MonkeyPatch,
):
    broadcasts: list[dict] = []
    cfg = _cfg()
    cfg.resume_text = "订单项目：使用 Redis 做热点缓存，并负责缓存一致性治理。"
    ...
    def fake_stream(_model, messages, **_kwargs):
        nonlocal model_called
        model_called = True
        assert "当前被问对象：Redis" in str(messages[-1]["content"])
        yield ("text", "做过，订单项目里我用 Redis 做过热点缓存，主要负责缓存一致性治理。")
    ...
    assert done["model_name"] == ""
    assert done["answer"].startswith("做过，")
    assert "缓存一致性治理" in done["answer"]


def test_personal_experience_with_evidence_rejects_model_topic_substitution(
    monkeypatch: pytest.MonkeyPatch,
):
    broadcasts: list[dict] = []
    cfg = _cfg()
    cfg.resume_text = "订单项目：使用 Redis 做热点缓存。"
    monkeypatch.setattr(answer_worker, "get_config", lambda: cfg)

    def fake_stream(_model, _messages, **_kwargs):
        yield ("text", "做过，我主要负责 Kafka 消息队列和线上吞吐优化。")
    ...
    assert "Kafka" not in done["answer"]
    assert "热点缓存" in done["answer"]


def test_unseen_experience_topic_calls_model_with_resume_context_and_keeps_adjacent_facts(
    monkeypatch: pytest.MonkeyPatch,
):
    broadcasts: list[dict] = []
    cfg = _cfg()
    cfg.resume_text = "负责订单系统开发，使用 MySQL 和 Redis。"
    monkeypatch.setattr(answer_worker, "get_config", lambda: cfg)
    ...
    monkeypatch.setattr(answer_worker, "build_system_prompt", fake_build_system_prompt)
    model_called = False

    def fake_stream(_model, _messages, **_kwargs):
        nonlocal model_called
        model_called = True
        yield ("text", "我没有直接做过 Kafka，但在订单系统里，我使用 MySQL 和 Redis。")

    monkeypatch.setattr(answer_worker, "chat_stream_single_model", fake_stream)

    answer_worker.process_question_parallel(
        ("你用过 Kafka 吗？", None, False, "asr", {"origin": "asr", "asr_turn_id": 1}),
        seq=0,
        model_idx=0,
        sess_v=0,
        deps=_deps(broadcasts=broadcasts),
    ...
    assert start["fact_grounding"]["status"] == "unsupported"
    assert start["model_name"] == "模型一"
    assert model_called is True
    assert prompt_calls[-1]["include_resume"] is True
    assert "没有直接做过 Kafka" in done["answer"]
    assert "MySQL" in done["answer"] and "Redis" in done["answer"]
...
def test_process_question_parallel_uses_concise_token_cap(monkeypatch: pytest.MonkeyPatch):
    broadcasts: list[dict] = []
    captured: dict[str, object] = {}
    ...
    def _concise_cfg():
        c = _cfg()
        c.assist_realtime_concise_answer = True
        return c

    monkeypatch.setattr(answer_worker, "get_config", _concise_cfg)

    answer_worker.process_question_parallel(
        ("Redis 怎么持久化？", None, False, "conversation_loopback", {"origin": "asr", "asr_turn_id": 3}),
        seq=0,
        model_idx=0,
        sess_v=0,
        deps=_deps(broadcasts=broadcasts),
    ...
    assert captured["override_max_tokens"] == 480

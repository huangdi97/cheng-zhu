...
def test_build_system_prompt_includes_high_churn_short_answer_instructions():
    # 本测试校验详细版/高 churn 版的指令，先关掉默认开启的极简话术模式。
    cfg = get_config()
    old_concise = cfg.assist_realtime_concise_answer
    cfg.assist_realtime_concise_answer = False
    try:
        ...
        cfg.assist_realtime_concise_answer = old_concise
...
def test_dedup_final_transcription_suppresses_repeats_within_window():
    from api.assist.pipeline import _dedup_final_transcription

    now = 100.0
    kept, dup = _dedup_final_transcription("请问你有项目经验吗", "", 0.0, now)
    assert kept == "请问你有项目经验吗" and dup is False
    kept, dup = _dedup_final_transcription("请问你有项目经验吗", kept, now, now + 0.3)
    assert kept == "" and dup is True
    kept, dup = _dedup_final_transcription("请问你有项目经验吗，具体讲讲", "", 0.0, now)
    assert kept != "" and dup is False
    # 后缀重叠（新文本是上一句的尾部，例如分句再次命中）-> 丢弃
    kept, dup = _dedup_final_transcription("具体讲讲", kept, now, now + 0.2)
    assert kept == "" and dup is True
    # 超过窗口 -> 允许
    kept, dup = _dedup_final_transcription("新问题", kept, now + 0.2, now + 2.0)
    assert kept == "新问题" and dup is False
    kept, dup = _dedup_final_transcription("", "x", now, now)
    assert kept == "" and dup is False

...
def test_append_transcription_fragment_broadcasts_and_feeds_auto_detect_when_unmerged():
    ...
    assert broadcasts[0] == {"type": "transcription", "text": "Kafka 如何保证顺序"}
    assert broadcasts[1]["type"] == "question_parse_status"
    assert broadcasts[1]["stage"] == "assembling"
...
def test_live_partial_extends_transcript_merge_window_for_long_speech():
    broadcasts: list[dict] = []
    state = AssistAsrStateMachine(
        broadcast=broadcasts.append,
        submit_answer_task=lambda _task: True,
    ...
    session = reset_session()
    cfg = _cfg(assist_transcription_merge_gap_sec=2.0)

    state.append_transcription_fragment(cfg, session, "为什么选择 Redis", now_mono=1.0)
    state.note_speech_activity(now_mono=2.8)
    state.try_flush_merge_buffer(cfg, session, now_mono=3.1)

    assert session.transcription_history == []
    state.append_transcription_fragment(cfg, session, "一致性怎么保证？如果挂了怎么办", now_mono=3.2)
    state.try_flush_merge_buffer(cfg, session, now_mono=5.3)

    assert len(session.transcription_history) == 1
    assert "Redis" in session.transcription_history[0]
    assert "一致性" in session.transcription_history[0]


def test_live_partial_extends_question_confirmation_window():
    submitted: list[tuple] = []
    state = AssistAsrStateMachine(
        broadcast=lambda _data: None,
        submit_answer_task=lambda task: submitted.append(task) or True,
    ...
    session = reset_session()
    cfg = _cfg(assist_asr_fast_confirm_sec=0.4, assist_asr_group_max_wait_sec=3.0)

    state.handle_auto_detect_asr_text(
        cfg, session, "为什么选择 Redis", "conversation_loopback", now_mono=1.0
    )
    state.note_speech_activity(now_mono=1.35)
    state.try_flush_question_group(cfg, session, now_mono=1.5)
...
def test_independent_questions_submit_multiple_tasks_in_one_asr_turn():
    submitted: list[tuple] = []
    state = AssistAsrStateMachine(
        broadcast=lambda _data: None,
        submit_answer_task=lambda task: submitted.append(task) or True,
        begin_asr_turn=lambda: 12,
        record_asr_turn=lambda _now: None,
        is_high_churn_submission=lambda _cfg, _now: False,
        logger=_Logger(),
        clock=lambda: 4.0,
    ...
    state.handle_auto_detect_asr_text(
        ...
        "conversation_loopback", 0.0,
    )
    state.try_flush_question_group(_cfg(), session, 1.0, True)

    assert len(submitted) == 2
    assert {task[4]["asr_turn_id"] for task in submitted} == {12}
    assert [task[4]["cluster_index"] for task in submitted] == [0, 1]
    assert all(task[4]["cluster_count"] == 2 for task in submitted)


def test_auto_answer_off_keeps_transcription_without_question_group():
    state = AssistAsrStateMachine(
        broadcast=lambda _data: None,
        submit_answer_task=lambda _task: True,
        begin_asr_turn=lambda: 1,
    ...
    session = reset_session()
    state.handle_auto_detect_asr_text(
        _cfg(assist_auto_answer_mode="off"), session,
        "Redis 为什么快", "conversation_loopback", 0.0,
    )

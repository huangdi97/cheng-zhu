...
def test_late_asr_constraint_tail_updates_deferred_pending_task(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(pipeline, "broadcast", lambda _data: None)
    # Default assist_answer_correction_enabled=True would call the LLM here;
    # make the correction a no-op so the test stays offline and deterministic.
    monkeypatch.setattr(
        "services.review_analysis.correct_question_text",
...
def test_high_confidence_structured_asr_skips_blocking_llm_correction(
    monkeypatch: pytest.MonkeyPatch,
):
    cfg = _cfg()
    cfg.assist_answer_correction_enabled = True
    monkeypatch.setattr(pipeline, "get_config", lambda: cfg)
    calls: list[str] = []
    monkeypatch.setattr(
        "services.review_analysis.correct_question_text",
        lambda question: calls.append(question) or question,
    )
    ...
    accepted = pipeline.submit_answer_task((
        ...
        "conversation_loopback",
        {
            "origin": "asr",
            "asr_turn_id": 1,
            "question_cluster": {"confidence": 0.9},
        },
    ))
...
def test_low_confidence_structured_asr_keeps_optional_llm_correction(
    monkeypatch: pytest.MonkeyPatch,
):
    cfg = _cfg()
    cfg.assist_answer_correction_enabled = True
    ...
    pipeline.submit_answer_task((
        ...
        "conversation_loopback",
        {
            "origin": "asr",
            "asr_turn_id": 1,
            "question_cluster": {"confidence": 0.55},
        },
    ))

    assert calls == ["Redis 一制性怎么办"]

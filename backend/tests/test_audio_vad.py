...
from services.audio import _windows_com_scope
...
def test_com_scope_is_safe_off_windows(monkeypatch):
    monkeypatch.setattr(audio_mod.platform, "system", lambda: "Linux")

    with _windows_com_scope():
        assert True


def test_windows_platform_info_reuses_already_listed_devices(monkeypatch):
    monkeypatch.setattr(audio_mod.platform, "system", lambda: "Windows")
    monkeypatch.setattr(
        AudioCapture,
        "list_devices",
        lambda: (_ for _ in ()).throw(AssertionError("must not enumerate twice")),
    )

    info = AudioCapture.get_platform_info(devices=[{"is_loopback": True}])

    assert info["has_loopback"] is True
    assert info["needs_virtual_device"] is False

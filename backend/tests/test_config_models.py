...
def test_config_payload_includes_realtime_voice_runtime_knobs():
    ...
    assert payload["assist_auto_answer_mode"] == "smart"


def test_auto_answer_mode_normalizes_and_disables_legacy_toggle():
    assert AppConfig(assist_auto_answer_mode="always").assist_auto_answer_mode == "always"
    disabled = AppConfig(assist_auto_answer_mode="off")
    assert disabled.auto_detect is False
    assert AppConfig(assist_auto_answer_mode="unexpected").assist_auto_answer_mode == "smart"
...
def test_transcription_preprocessing_flags_defaults_and_clamp():
    cfg = AppConfig()
    assert cfg.assist_answer_correction_enabled is True
    assert cfg.assist_question_cleanup_enabled is True
    assert cfg.assist_realtime_think_enabled is False

    cfg2 = AppConfig(
        assist_answer_correction_enabled=False,
        assist_question_cleanup_enabled=False,
        assist_realtime_think_enabled=True,
    )
    assert cfg2.assist_answer_correction_enabled is False
    assert cfg2.assist_question_cleanup_enabled is False
    assert cfg2.assist_realtime_think_enabled is True

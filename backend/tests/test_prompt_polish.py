...
def _asr_prompt(high_churn: bool = False, concise: bool = False) -> str:
    cfg = get_config()
    old = cfg.assist_realtime_concise_answer
    cfg.assist_realtime_concise_answer = concise
    try:
...
def test_prompt_locks_yes_no_experience_question_to_factual_direct_answer():
    p = _asr_prompt(high_churn=False)

    assert "问题忠实度硬规则" in p
    assert "只回答这一问" in p
    assert "不得把是否题改答成技术原理或通用方案" in p
    assert "知识掌握、个人练习、有限接触和生产项目经验必须区分" in p
...
def test_asr_realtime_concise_mode_is_short_and_oral():
    p = _asr_prompt(concise=True)
    assert "照着就能说出口" in p
    assert "100-150 字" in p
    assert "不要「这是一个很好的问题」" in p
    assert "不输出 `##` 标题" in p
    assert "专注面板输出协议" not in p
    assert "3-5 个 section" not in p
    assert "问题忠实度硬规则" in p
    # 详细模式仍在 concise 关闭时保留
    detailed = _asr_prompt(concise=False)
    assert "详细回答模式" in detailed or "输入判定三档 ladder" in detailed

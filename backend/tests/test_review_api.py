...
def test_update_and_delete_review_turn(monkeypatch):
    started: list[int] = []
    monkeypatch.setattr(
        review_router.review_async_analysis, "analyze_session_async", lambda sid: started.append(sid)
    )
    sid = review.create_session(
        started_at=time.time(),
        interviewer_enabled=True,
        candidate_enabled=True,
        source="manual",
    ...
    resp = client.patch(
        f"/api/review/sessions/{sid}/turns/m-1",
        json={"question_text": "q2", "candidate_answer_text": "a2", "analyze": True},
    )
    assert resp.status_code == 200
    turn = review.get_session_detail(sid)["turns"][0]
    assert turn["question_text"] == "q2"
    assert turn["candidate_answer_text"] == "a2"
    assert turn["analysis_status"] == "pending"
    assert started == [sid]

    resp = client.delete(f"/api/review/sessions/{sid}/turns/m-1?analyze=True")
    assert resp.status_code == 200
    assert review.get_session_detail(sid)["turns"] == []

...
# ---------------------------------------------------------------------------
# cleanup_asr_question_text (pre-LLM fast cleanup)
# ---------------------------------------------------------------------------

def test_cleanup_strips_leading_and_trailing_fillers():
    from services.stt.text_utils import cleanup_asr_question_text

    assert cleanup_asr_question_text("那个Redis和MySQL的区别是什么") == "Redis和MySQL的区别是什么"
    assert cleanup_asr_question_text("嗯呃那个，就是说，你讲一下你的项目") == "你讲一下你的项目"
    assert cleanup_asr_question_text("你先说说你的项目然后") == "你先说说你的项目"
    assert cleanup_asr_question_text("好的好的，那么你觉得呢") == "你觉得呢"


def test_cleanup_removes_doubled_interjections_and_repeat_punct():
    from services.stt.text_utils import cleanup_asr_question_text

    assert cleanup_asr_question_text("嗯嗯，然后呢？？？") == "然后呢？"
    assert cleanup_asr_question_text("啊啊啊，这个怎么实现") == "怎么实现"


def test_cleanup_preserves_meaningful_text_and_tech_terms():
    from services.stt.text_utils import cleanup_asr_question_text

    assert cleanup_asr_question_text("Kafka 和 RabbitMQ 的区别是什么") == "Kafka 和 RabbitMQ 的区别是什么"
    assert cleanup_asr_question_text("金额的校验怎么做") == "金额的校验怎么做"
    assert cleanup_asr_question_text("") == ""

...
def test_provider_402_balance_error_has_actionable_user_message():
    error = RuntimeError("Error code: 402 - Insufficient Balance")

    classified = _classify_exception(error)

    assert classified.user_msg == "大模型账户余额不足，请充值或切换可用模型。"


def test_deepseek_v4_generic_reasoning_marker():
    from services.llm.streaming import _is_generic_reasoning_model, _disabled_think_params_for_model
    ...
    assert _is_generic_reasoning_model(m) is True
    params = _disabled_think_params_for_model(m, "generic")

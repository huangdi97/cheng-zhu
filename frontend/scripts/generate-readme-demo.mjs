...
async function runCommand(command, args, cwd) {
  const child = spawn(command, args, {
    cwd,
    stdio: 'inherit',
    // Windows exposes npm as npm.cmd; shell mode keeps the generator
    // portable across Windows and POSIX environments.
...
3. 线上通常会同时开启，再结合主从和故障转移一起看，才更接近真实生产结论。`
  ...
  if (await settingsButton.count().catch(() => 0)) {
    await setCaption(page, '桌面端把快捷键、托盘和悬浮提示窗收在一起；窗口隐私保护只是尽量减少本机意外捕获。', '06 / DESKTOP')
    ...
    if (await searchInput.count().catch(() => 0)) {
      ...
      await page.getByText('窗口隐私保护说明').waitFor({ timeout: 4000 }).catch(() => {})
      await captureFrame(page, frames, BEAT)
      await setCaption(page, '隐私保护依赖操作系统和应用支持，成竹不会承诺绕过会议、监考或反作弊策略；请按你的环境验证。', '07 / PRIVACY')
  ...
  await setCaption(page, '实时转录、自动回答、问题切分、知识库引用和可定制的 LLM 参数，组成了成竹的核心练习闭环。', 'END / SUMMARY')

...
const OUTPUT_DIR = process.env.CZ_README_SCREENSHOT_OUTPUT_DIR
  ? path.resolve(process.env.CZ_README_SCREENSHOT_OUTPUT_DIR)
  : path.resolve(REPO_DIR, 'docs', 'screenshots')
const MOBILE_CAPTURE = process.env.CZ_README_SCREENSHOT_MOBILE === '1'
...
`,
      ...
      question_cluster: {
        primary: '请介绍 Redis 持久化，并比较 RDB 和 AOF',
        subquestions: ['两种机制如何取舍', '故障恢复时怎么选', '为什么这样选'],
        constraints: ['结合线上故障恢复场景'],
        question_type: 'system_design',
        confidence: 0.93,
      },
      cluster_index: 0,
...
].filter((shot) => !process.env.CZ_README_SCREENSHOT_ONLY
  || shot.scenario === process.env.CZ_README_SCREENSHOT_ONLY)
  ...
  const scenarioMessages = {
    assist: [
      ...
      { type: 'audio_level', value: 0.62, delay: 80 },
      {
        ...
        stage: 'parsed',
        message: '识别为同一主题下的 1 个主问题和 3 个追问',
        raw_text: '请介绍 Redis 持久化，并比较 RDB 和 AOF。再说说故障恢复时怎么选，为什么？',
        clusters: [{
          primary: '请介绍 Redis 持久化，并比较 RDB 和 AOF',
          subquestions: ['两种机制如何取舍', '故障恢复时怎么选', '为什么这样选'],
          question_type: 'system_design',
...
async function preparePage(browser, baseUrl, scenario) {
  const context = await browser.newContext({
    viewport: MOBILE_CAPTURE ? { width: 390, height: 844 } : { width: 1720, height: 960 },
    deviceScaleFactor: 1,
  })
  ...
  await context.addInitScript(({ scenario: shotScenario, messages }) => {
    ...
    window.__IA_EMIT_WS__ = (payload) => {
      liveSockets.forEach((socket) => {
        if (socket.readyState !== MockWebSocket.OPEN || typeof socket.onmessage !== 'function') return
...
async function captureAssist(page, outputPath) {
  await page.getByRole('heading', { name: '成竹 · 面试工作台', exact: true }).waitFor({ timeout: 5000 })
  await page.getByRole('button', { name: /实时辅助/ }).click()
  await page.getByRole('button', { name: '语音模式' }).waitFor({ timeout: 5000 })
  await page.evaluate(() => window.__IA_EMIT_WS__?.({
    type: 'question_parse_status',
    stage: 'parsed',
    message: '识别为同一主题下的 1 个主问题和 3 个追问',
    raw_text: '请介绍 Redis 持久化，并比较 RDB 和 AOF。再说说故障恢复时怎么选，为什么？',
...
async function main() {
  ...
  const executablePath = process.env.CZ_PLAYWRIGHT_EXECUTABLE
    || (SYSTEM_CHROME && existsSync(SYSTEM_CHROME) ? SYSTEM_CHROME : undefined)

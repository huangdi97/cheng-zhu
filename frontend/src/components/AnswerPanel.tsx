    ...
    return (
      <div className={`space-y-3 ${baseClass}`} style={animStyle}>
        <div className="flex items-start gap-3">
          <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-accent-blue/25 to-accent-blue/15 flex items-center justify-center flex-shrink-0 text-accent-blue text-xs font-bold ring-1 ring-accent-blue/15">
            Q
          </div>
          <div className="flex-1 min-w-0 pt-1">
            <div className="flex flex-wrap items-center gap-1.5 mb-1">
              ...
              {qa.questionType && (
                <span className="rounded-full border border-accent-blue/25 bg-accent-blue/10 px-1.5 py-0.5 text-[10px] font-medium text-accent-blue">
                  {QUESTION_TYPE_LABELS[qa.questionType] || qa.questionType}
                </span>
              )}
              {(qa.clusterCount ?? 0) > 1 && (
                <span className="rounded-full border border-bg-hover bg-bg-tertiary px-1.5 py-0.5 text-[10px] text-text-muted">
                  问题 {(qa.clusterIndex ?? 0) + 1}/{qa.clusterCount}
                </span>
              )}
              {isStreaming && (
            ...
            {qa.questionCluster?.subquestions?.length ? (
              <p className="mt-1 text-[10px] text-text-muted">
                回答清单：{qa.questionCluster.subquestions.map((item) => item.replace(/[？?]$/, '')).join(' · ')}
              </p>
            ) : null}
          </div>
        </div>
  ...
  return (
    <div className={`answer-card p-4 space-y-3 ${baseClass}`} style={animStyle}>
      <div className="flex items-start gap-2.5">
        <div className="w-7 h-7 rounded-xl bg-gradient-to-br from-accent-blue/25 to-accent-blue/15 flex items-center justify-center flex-shrink-0 mt-0.5 ring-1 ring-accent-blue/15">
      ...
      <div className="flex items-start gap-2.5">
        <div className="w-7 h-7 rounded-xl bg-gradient-to-br from-accent-green/25 to-accent-green/15 flex items-center justify-center flex-shrink-0 mt-0.5 ring-1 ring-accent-green/15">
      ...
      {suggestion && !isStreaming && (
        <div className="ml-9 rounded-xl border border-accent-amber/25 bg-accent-amber/5 px-3 py-2.5">
          <div className="flex items-center gap-1.5 text-[10px] font-semibold text-accent-amber mb-1.5">
            <span className="w-1.5 h-1.5 rounded-full bg-accent-amber" />
            面试教练建议
          </div>
          <pre className="whitespace-pre-wrap font-sans text-xs text-text-secondary leading-relaxed">{suggestion.text}</pre>
        </div>
      )}
    </div>
  )
  ...
  return (
    <div className="relative h-full min-h-0 flex flex-col">
      <div
        ...
        className={`flex-1 min-h-0 overflow-y-auto px-4 py-4 md:px-5 md:py-5 ${stream ? 'space-y-10' : 'space-y-5'}`}
      >
      {multiStream && (

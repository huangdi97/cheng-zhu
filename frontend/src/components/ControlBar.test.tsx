...
const updateConfigAndRefreshMock = vi.hoisted(() => vi.fn())
vi.mock('@/lib/configSync', () => ({
  updateConfigAndRefresh: updateConfigAndRefreshMock,
}))
...
describe('ControlBar', () => {
  ...
  it('offers off, smart and always automatic-answer modes', async () => {
    render(<ControlBar />)
    fireEvent.click(screen.getByRole('button', { name: '更多操作' }))
    fireEvent.click(screen.getByRole('menuitemradio', { name: '全自动' }))

    await waitFor(() => expect(updateConfigAndRefreshMock).toHaveBeenCalledWith({

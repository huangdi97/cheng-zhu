import { Pause, AlertTriangle, Info, CheckCircle2, XCircle, X } from 'lucide-react'
      ...
      {!wsIsLeader && (
        <div className="animate-fade-up">
          <div className="glass border border-accent-amber/30 text-text-primary text-xs px-4 py-2.5 rounded-xl shadow-xl shadow-black/20 flex items-center gap-3">
            <Pause className="w-3.5 h-3.5 text-accent-amber flex-shrink-0" aria-hidden />
      ...
      {fallbackToast && (
        <div className="animate-fade-up" role="alert" aria-live="assertive">
          <div className="glass border border-accent-amber/30 text-text-primary text-xs px-4 py-2.5 rounded-xl shadow-xl shadow-black/20">
            <AlertTriangle className="w-3.5 h-3.5 text-accent-amber flex-shrink-0" aria-hidden /> <span>{fallbackToast.from} 不可用，切换到 {fallbackToast.to}</span>
          </div>
        </div>
      )}
      {toasts.map((toast) => {
        ...
        const Icon = { info: Info, success: CheckCircle2, warn: AlertTriangle, error: XCircle }[toast.level]
        const iconCls = {
        ...
        return (
          <div key={toast.id} className="animate-fade-up" role={role} aria-live={live}>
            ...
            >
              <Icon className={`w-3.5 h-3.5 flex-shrink-0 ${iconCls}`} aria-hidden />

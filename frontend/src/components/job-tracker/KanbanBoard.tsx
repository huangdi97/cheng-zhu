  ...
  return (
    <select
      className={`mt-2 w-full max-w-full rounded-lg border px-2 py-1.5 text-[11px] font-medium outline-none transition-colors ${
        isLight
          ? 'border-bg-hover bg-bg-primary text-text-primary focus:border-accent-blue/50 focus:ring-1 focus:ring-accent-blue'
  ...
  const timeBadgeClass = terminalStage
    ? rejectedStage
      ? isLight
        ...
        : 'bg-accent-red/15 text-accent-red/80'
      : isLight
        ? 'bg-bg-tertiary text-text-secondary border border-bg-hover'
        : 'bg-white/[0.06] text-text-muted'
    : isLight
      ? 'bg-accent-amber/10 text-accent-amber border border-accent-amber/30'
      : 'bg-accent-amber/15 text-accent-amber/80'
  ...
  return (
    <div
      ...
      className={`
        ...
        ${leftBorder}
        ${isLight
          ? 'border-bg-hover/80 bg-bg-primary'
      ...
      <div className="flex gap-2">
        <button
          ...
          className={`
            ...
            ${isLight ? 'text-text-muted hover:bg-bg-tertiary' : 'text-text-muted/60 hover:bg-white/[0.06]'}
          `}
          aria-label="\u62D6\u52A8\u6392\u5E8F"
          disabled={sortDisabled || busy}
          {...attributes}
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start gap-2">
            ...
            <div className="min-w-0 flex-1">
              <p className={`text-[13px] font-semibold leading-snug line-clamp-2 ${isLight ? 'text-text-primary' : 'text-text-primary'}`}>
                {app.company}
              </p>
              <p className={`mt-0.5 flex items-start gap-1 text-[11px] ${isLight ? 'text-text-secondary' : 'text-text-secondary'}`}>
          ...
          <div className="flex items-center gap-2 mt-1.5 pl-[2.5rem] flex-wrap">
            {app.city ? (
              <span className={`inline-flex items-center gap-1 text-[10px] ${isLight ? 'text-text-muted' : 'text-text-muted'}`}>
              ...
              <span
                className={`inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[10px] font-medium ${
                  reviewSummary.review_count <= 0
                    ...
                    : reviewScore == null
                    ? isLight ? 'bg-bg-tertiary text-text-secondary' : 'bg-white/[0.06] text-text-muted'
                    : reviewScore < 6
                      ? 'bg-accent-amber/15 text-accent-amber'
                      : reviewScore >= 8
            ...
            {nextStage && !busy && (
              <button
                ...
                className={`mt-2 inline-flex h-[30px] shrink-0 items-center justify-center gap-1 rounded-lg border px-2 transition-all ${
                  isLight
                    ? 'border-bg-hover bg-bg-secondary text-text-secondary hover:bg-accent-blue/10 hover:text-accent-blue hover:border-accent-blue/30'
  ...
  return (
    <div
      ...
      className={`
        ...
        ${isLight
          ? 'border-bg-hover bg-bg-secondary/80'
    ...
    >
      ...
      <div
        className={`flex items-center gap-2.5 border-b px-3 py-2.5 shrink-0 ${
          isLight ? 'border-bg-hover bg-white/60' : 'border-white/[0.06] bg-black/10'
        }`}
      >
        <div
        ...
        <div className="min-w-0 flex-1">
          <h3 className={`truncate text-xs font-bold ${isLight ? 'text-text-primary' : 'text-text-primary'}`}>
            <span>{STAGE_LABELS[stage] ?? stage}</span>
          </h3>
          <p className={`text-[10px] mt-0.5 ${isLight ? 'text-text-muted' : 'text-text-muted'}`}>
            {list.length}{' \u6761\u8BB0\u5F55'}
          </p>
        </div>
        <span
      ...
      <div data-kanban-column-scroll className="flex flex-1 flex-col gap-2 overflow-y-auto p-2.5 [scrollbar-width:thin]">
        ...
        {list.length === 0 && (
          <div className="flex flex-1 flex-col items-center justify-center gap-2 py-12 text-center">
            <div className={`rounded-lg p-3 ${isLight ? 'bg-bg-tertiary' : 'bg-white/[0.03]'}`}>
              <Inbox className={`w-8 h-8 ${isLight ? 'text-text-muted' : 'text-text-muted/30'}`} strokeWidth={1.5} />
            </div>
            <p className={`text-[11px] ${isLight ? 'text-text-muted' : 'text-text-muted/60'}`}>{'\u65E0\u5339\u914D\u8BB0\u5F55'}</p>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div
      className={`
        ...
        ${leftBorder}
        ${isLight ? 'border-bg-hover bg-bg-primary' : 'border-white/20 bg-bg-tertiary'}
      `}
    >
      <div className="flex items-center gap-2">
        ...
        <p className={`text-[13px] font-semibold line-clamp-1 ${isLight ? 'text-text-primary' : 'text-text-primary'}`}>{app.company}</p>
      </div>
      <p className={`mt-1 text-[11px] line-clamp-1 ${isLight ? 'text-text-secondary' : 'text-text-secondary'}`}>{app.position || '\u5C97\u4F4D'}</p>
    </div>
  )
}
  ...
  return (
    <div
      className={`flex h-full min-h-0 gap-3 rounded-lg border p-3 md:p-4 ${
        isLight ? 'border-bg-hover bg-white/60' : 'border-white/[0.07] bg-bg-primary/40'
      }`}
    >
      {/* Left stage rail (desktop) */}
      <nav
        className={`hidden w-14 shrink-0 flex-col gap-1 md:flex ${
          isLight ? 'border-r border-bg-hover pr-2' : 'border-r border-white/[0.06] pr-2'
        }`}
        aria-label="\u9636\u6BB5\u5BFC\u822A"
      >
        {visibleStages.map((st) => {
          ...
          return (
            <button
              ...
              className={`
                relative flex flex-col items-center gap-0.5 rounded-xl py-2 text-[10px] font-bold transition-all
                ${isLight
            ...
            >
              ...
              {n > 0 ? (
                <span
                  className={`absolute -right-0.5 -top-0.5 flex h-4 min-w-[1rem] items-center justify-center rounded-full px-1 text-[10px] font-bold ${theme.dotColor} text-white shadow-sm`}
                >
                  {n > 99 ? '99+' : n}
                </span>
              ) : null}
          ...
          <div className="flex items-center gap-3">
            <div
              className={`flex h-9 w-9 items-center justify-center rounded-lg ${
                isLight ? 'bg-accent-blue/10 text-accent-blue' : 'bg-accent-blue/20 text-accent-blue/80'
              }`}
            >
              <LayoutGrid className="w-5 h-5" strokeWidth={2} />
            </div>
            <div>
              <h2 className={`text-sm font-bold tracking-tight ${isLight ? 'text-text-primary' : 'text-text-primary'}`}>
                {'\u6C42\u804C\u7BA1\u9053'}
              </h2>
              <p className={`text-[10px] mt-0.5 ${isLight ? 'text-text-muted' : 'text-text-muted'}`}>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {!showTerminalStages && terminalApplicationsCount > 0 ? (
              <button
                ...
                className={`rounded-full px-2.5 py-1 text-[10px] font-semibold ring-1 transition-colors ${
                  isLight
                    ? 'bg-accent-red/10 text-accent-red ring-accent-red hover:bg-accent-red/10'
                    : 'bg-accent-red/15 text-accent-red/80 ring-accent-red/25 hover:bg-accent-red/25'
                }`}
              >
            ...
            <label
              className={`flex cursor-pointer items-center gap-2 rounded-lg border px-2.5 py-1.5 text-[11px] font-medium transition-colors ${
                isLight
                  ? 'border-bg-hover bg-bg-primary hover:bg-bg-secondary'
            ...
            <div className={`flex rounded-lg border p-0.5 ${isLight ? 'border-bg-hover' : 'border-bg-hover'}`}>
              <button
                type="button"
                aria-label="\u5411\u5DE6"
                className={`rounded-lg p-1.5 transition-colors ${isLight ? 'text-text-muted hover:bg-bg-tertiary' : 'text-text-muted hover:bg-bg-hover'}`}
            ...
            return (
              <button
                ...
                className={`shrink-0 rounded-full px-3 py-1.5 text-[10px] font-semibold whitespace-nowrap flex items-center gap-1.5 transition-colors ${
                  isLight
                    ? 'bg-bg-primary border border-bg-hover text-text-secondary'

...
function loaderIcon(loader: string) {
  if (loader === 'pdf') return <FileType2 className="w-3.5 h-3.5 text-accent-red" />
  if (loader === 'docx') return <FileText className="w-3.5 h-3.5 text-accent-blue" />
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        <button
          ...
          disabled={uploading}
          className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium bg-accent-amber/15 border border-accent-amber/30 text-accent-amber/80 hover:bg-accent-amber/25 hover:border-accent-amber/50 transition-colors disabled:opacity-50"
        >
      ...
      {!hasDocs ? (
        <div
          ...
          className={`group relative cursor-pointer rounded-2xl border-2 border-dashed transition-all px-5 py-10 text-center overflow-hidden focus:outline-none focus:ring-2 focus:ring-accent-amber/40 ${
            dragOver
              ? 'border-accent-amber/70 bg-gradient-to-b from-accent-amber/15 to-accent-amber/5 scale-[1.01]'
              : 'border-bg-hover/50 bg-gradient-to-b from-bg-tertiary/30 to-bg-tertiary/10 hover:border-accent-amber/50 hover:from-accent-amber/8 hover:to-transparent'
          }`}
        >
          <div
            className={`mx-auto mb-3 w-14 h-14 rounded-2xl flex items-center justify-center transition-all ${
              dragOver
                ? 'bg-accent-amber/25 scale-110'
                : 'bg-accent-amber/25 group-hover:bg-accent-amber/25 group-hover:scale-105'
            }`}
          >
            <UploadCloud
              className={`w-7 h-7 transition-colors ${
                dragOver ? 'text-accent-amber/80' : 'text-accent-amber group-hover:text-accent-amber/80'
              }`}
              strokeWidth={1.6}
            />
          </div>
          ...
          <div className="text-[11px] text-text-muted mb-4">
            或点击选择 · 答题时会以 <span className="text-accent-amber font-medium">[角标]</span> 引用并显示来源
          </div>

...
const ORIGIN_COLOR: Record<string, string> = {
  ...
  ocr: 'border-accent-amber/40 bg-accent-amber/10 text-accent-amber',
  vision: 'border-accent-blue/40 bg-accent-blue/10 text-accent-blue',
  mixed: 'border-accent-blue/40 bg-accent-blue/10 text-accent-blue',
}
    ...
    <div className="space-y-3">
      <form onSubmit={run} className="flex gap-2">
        <input
          ...
          className="flex-1 px-2.5 py-1.5 rounded-lg bg-bg-tertiary/60 border border-bg-hover/40 text-xs text-text-primary placeholder:text-text-muted focus:outline-none focus:border-accent-amber/60"
        />
        <input
          type="number"
          value={k}
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium bg-accent-amber/15 border border-accent-amber/30 text-accent-amber/80 hover:bg-accent-amber/25 disabled:opacity-50"
        >
        ...
        {hits.map((h, i) => (
          ...
          >
            <div className="flex items-center gap-1.5 mb-1 flex-wrap">
              ...
              <span className="text-xs text-accent-amber/80 font-medium truncate">
                {h.section_path || '(无小节)'}
              </span>
              <span

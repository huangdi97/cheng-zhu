    ...
    >
      ...
      {!enabled && (
        <button
          ...
          className="w-full flex items-center justify-between gap-2 px-3 py-2 border-t border-accent-amber/20 bg-accent-amber/[0.06] hover:bg-accent-amber/[0.12] transition-colors text-left group"
        >
          <span className="flex items-center gap-2 min-w-0 text-[10.5px] text-text-secondary leading-snug">
            <Settings2 className="w-3.5 h-3.5 flex-shrink-0 text-accent-amber" />
            <span className="truncate">
              主流程不引用本地笔记。
              <span className="text-accent-amber font-medium">点此打开设置中心 →</span>
            </span>
          </span>
          <span className="text-[10px] text-text-muted group-hover:text-accent-amber flex-shrink-0 transition-colors">
            偏好 · 知识库
          </span>
        </button>
      )}

...
type SpeechForm = {
  ...
  auto_detect: boolean
  assist_auto_answer_mode: 'off' | 'smart' | 'always'
}
...
const DEFAULT_SPEECH_FORM: SpeechForm = {
  ...
  candidate_streaming_asr_interval_ms: 400,
...
function speechFormFromConfig(config: AppConfig): SpeechForm {
  return {
    ...
    candidate_streaming_asr_interval_ms: clampIntegerInput(config.candidate_streaming_asr_interval_ms, 300, 5000, DEFAULT_SPEECH_FORM.candidate_streaming_asr_interval_ms),
    ...
    assist_auto_answer_mode: config.assist_auto_answer_mode ?? (config.auto_detect === false ? 'off' : 'smart'),
  }
}
...
export default function SpeechTab() {
  ...
  const handleSave = async () => {
    ...
    try {
      const savedForm = {
        ...form,
        auto_detect: form.assist_auto_answer_mode !== 'off',
        ...
        candidate_streaming_asr_interval_ms: clampIntegerInput(form.candidate_streaming_asr_interval_ms, 300, 5000, 400),
  ...
  const brandBorder: Record<string, string> = {
    whisper: 'border-accent-blue/40',
    doubao: 'border-accent-amber/40',
    generic: 'border-accent-blue/40',
  }
  ...
  return (
    <div className="p-5 space-y-5 pb-8" data-in-search={inSearch ? '1' : undefined}>
          ...
          {providers.map((p) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-[10px] text-text-muted leading-snug">{meta.desc}</span>
                {/* credential status dot */}
                <span className={`absolute top-2 right-2 flex items-center gap-1`}>
                  {cred
                    ? <CheckCircle2 className="w-3 h-3 text-accent-green" />
                    : p !== 'whisper' ? <XCircle className="w-3 h-3 text-accent-red" /> : null}
                </span>
              </button>
            )
          })}
      ...
      {!providerSupported && (
        <div className="rounded-xl border border-accent-amber/30 bg-accent-amber/10 px-4 py-3 text-xs text-accent-amber/80">
        ...
        <GradientCard className="p-4 space-y-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="inline-flex shrink-0 items-center gap-2 self-start rounded-lg border border-bg-hover bg-bg-tertiary/45 px-2.5 py-2">
              <span className={`text-xs font-medium ${form.candidate_asr_enabled ? 'text-accent-green' : 'text-text-muted'}`}>
              ...
              <button
                ...
                className={`relative h-6 w-11 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-accent-blue/30 ${
                  form.candidate_asr_enabled ? 'bg-accent-green/80' : 'bg-bg-hover'
                }`}
              >
                <span
          ...
          {form.candidate_asr_enabled && (
            <div className="rounded-lg border border-accent-green/20 bg-accent-green/10 px-3 py-2 text-xs leading-relaxed text-text-secondary">
          ...
          {form.candidate_asr_enabled && form.candidate_stt_provider !== 'whisper' && (
            <div className={`rounded-lg border px-3 py-2 text-xs leading-relaxed ${
              candidateRemoteCredentialReady
                ? 'border-accent-amber/25 bg-accent-amber/10 text-text-secondary'
                : 'border-accent-red/30 bg-accent-red/10 text-accent-red/80'
            }`}>
              {candidateRemoteCredentialReady
              ...
              <div className="grid gap-3 md:grid-cols-4">
                <Field label="边写刷新间隔 (ms)" hint="每隔多久尝试把你正在说的话写进上下文。拿不到本地 Whisper 空闲资源时会跳过本次，优先保证面试官 ASR。">
                  <input
                    ...
                    onChange={(e) => setForm({ ...form, candidate_streaming_asr_interval_ms: clampIntegerInput(e.target.value, 300, 5000, 400) })}
                    className="input-field"
      ...
      <Section title="转写合并与自动答题" keywords="合并 merge auto answer 自动答题 gap interval seconds 间隔">
        ...
        <Field label="自动回答模式" hint="智能模式会过滤陈述和寒暄；全自动适合问答节奏很快的场景。">
          <div className="grid grid-cols-3 gap-2">
            {([
              { value: 'off' as const, label: '关闭', hint: '只转录' },
              { value: 'smart' as const, label: '智能', hint: '推荐' },
              { value: 'always' as const, label: '全自动', hint: '有效问句直答' },
            ]).map((item) => {
              const selected = form.assist_auto_answer_mode === item.value
              return (
                <button
                  key={item.value}
                  type="button"
                  onClick={() => setForm({
                    ...form,
                    assist_auto_answer_mode: item.value,
                    auto_detect: item.value !== 'off',
                  })}
                  className={`rounded-xl border px-2.5 py-2 text-left transition-colors ${
                    selected
                      ? 'border-accent-blue bg-accent-blue/10 text-accent-blue'
                      : 'border-bg-hover bg-bg-tertiary/25 text-text-secondary hover:text-text-primary'
                  }`}
                >
                  <span className="block text-xs font-semibold">{item.label}</span>
                  <span className="mt-0.5 block text-[10px] text-text-muted">{item.hint}</span>
                </button>
              )
            })}
          </div>

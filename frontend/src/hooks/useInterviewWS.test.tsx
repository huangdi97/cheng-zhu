...
describe('useInterviewWS', () => {
  ...
  it('surfaces realtime STT stream failures while batch recovery continues', () => {
    render(<Harness />)
    const ws = FakeWebSocket.instances[0]
    ...
    act(() => {
      ...
      ws.emitMessage({
        ...
        provider: 'doubao',
        message: '豆包实时转录连接中断，将在分句后自动重试',
      })
    })

    expect(useInterviewStore.getState().toastMessage).toContain('将在分句后自动重试')
  })

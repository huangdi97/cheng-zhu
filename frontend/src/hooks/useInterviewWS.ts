  ...
  const handleMessage = (msg: WsMsg) => {
    ...
    switch (msg.type) {
      ...
      case 'audio_level':
        ...
        break
      case 'candidate_audio_level':
        s.setCandidateAudioLevel(msg.value as number)
        break
      case 'transcribing':
        s.setTranscribing(msg.value as boolean)
        break
      case 'interviewer_transcription_partial':
        s.setInterviewerPartial(msg.text as string)
        break
      case 'stt_stream_error':
        s.pushToast(String(msg.message || '实时转录连接中断，正在尝试分句识别'), 'warn', 7000)
        break
      case 'transcription':
        s.addTranscription(msg.text as string)
        s.setInterviewerPartial(null)
        break
      case 'candidate_transcription_partial':
        s.setCandidatePartial(msg.text as string)
        break
      case 'candidate_transcription':
        ...
        s.setCandidatePartial(null)
        break
      case 'copilot_hint':
        s.setCopilotHint(msg as Parameters<typeof s.setCopilotHint>[0])
        break
      case 'question_parse_status':
        s.setQuestionParseStatus(msg as unknown as Parameters<typeof s.setQuestionParseStatus>[0])
        break
      ...
      case 'answer_start':
        ...
        s.startAnswer(msg.id as string, msg.question as string, {
          ...
          questionType: msg.question_type as string | undefined,
          questionCluster: msg.question_cluster as any,
          clusterIndex: msg.cluster_index as number | undefined,
          clusterCount: msg.cluster_count as number | undefined,
        })
        const parsedQuestion = useInterviewStore.getState().questionParseStatus
        s.setQuestionParseStatus({
          ...parsedQuestion,
          stage: 'answering',
          message: '问题已确认，正在生成回答',
          clusters: parsedQuestion?.clusters
      ...
      case 'answer_done':
        ...
        // TTS 语音播报（可选）：答案完成后朗读
        import('@/lib/tts').then(({ speak }) => speak(String(msg.answer ?? '')))
        break
      case 'memo_update':
        s.setMemo({
          system_summary: msg.system_summary as string | undefined,
          memo_manual: msg.memo_manual as string | undefined,
        })
        break
      case 'translation':
        s.addTranslation(
          msg.question as string,
          msg.translated as string,
          msg.to as string,
        )
        break
      case 'suggestion':
        s.addSuggestion(msg.id as string, msg.text as string, msg.question as string)
        break
      case 'sessions_changed':
        s.setSessions((msg.items as Parameters<typeof s.setSessions>[0]) ?? [], msg.active_id as string)
        break

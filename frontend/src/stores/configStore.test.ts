...
describe('configStore transcription windows', () => {
  ...
  it('restores structured question metadata from session init data', () => {
    useInterviewStore.getState().setInitData({
      transcriptions: [],
      qa_pairs: [{
        id: 'qa-cluster',
        question: '请比较 RDB 和 AOF',
        answer: '先给结论。',
        question_type: 'system_design',
        question_cluster: {
          primary: '请比较 RDB 和 AOF',
          subquestions: ['故障恢复时怎么选'],
        },
        cluster_index: 0,
        cluster_count: 2,
      }],
    ...
    const qa = useInterviewStore.getState().qaPairs[0]
    expect(qa.questionType).toBe('system_design')
    expect(qa.questionCluster?.subquestions).toEqual(['故障恢复时怎么选'])
    expect(qa.clusterIndex).toBe(0)
    expect(qa.clusterCount).toBe(2)
  })
})

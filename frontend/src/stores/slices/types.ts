...
export interface AppConfig {
  ...
  assist_answer_align_jd_enabled?: boolean
  assist_inline_translation_enabled?: boolean
  assist_suggestion_enabled?: boolean
  assist_prefix_cache_warmup_enabled?: boolean
  rolling_memo_enabled?: boolean
  auto_detect: boolean
  assist_auto_answer_mode?: 'off' | 'smart' | 'always'
  silence_threshold: number
  silence_duration: number
  /** ????????????? LLM ?? ASR ?????????? */
  assist_answer_correction_enabled?: boolean
  api_key_set: boolean
  has_resume: boolean
  ...
  /** 默认短口语回答；开放观点/设计/追问由后端自动切换为深度结构 */
  assist_realtime_concise_answer?: boolean
  /** 自动回答前用 LLM 判题，拦截误把陈述句当提问 */
  assist_llm_question_detect_enabled?: boolean
  /** 极简话术模式输出 token 上限 */
  assist_realtime_concise_max_tokens?: number
  /** 实时语音问答默认输出 token 上限 */
